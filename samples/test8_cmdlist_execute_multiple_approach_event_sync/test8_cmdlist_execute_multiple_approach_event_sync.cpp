...
int main(int argc, char* argv[])
{
    ...
    if (file.is_open()) {
        ...
        for (size_t i = 0; i < sizeof(kernels); i++)
        {
            // Create kernel
        ...
        const uint32_t kernelArrSize = sizeof(kernels);
        printf("We might get here...");
        VALIDATECALL(zeCommandListAppendLaunchMultipleKernelsIndirect(cmdList, 2, kernels, &kernelArrSize, &launchArgs, nullptr, 0, nullptr));
    ...
    */
    ...
    //file.clear();
    //file.open("matrixMultiply.spv", std::ios::binary); <-- Kommentit pois kun ajetaan taas
    if (file.is_open()) {
        file.seekg(0, file.end);
        auto length = file.tellg();
        file.seekg(0, file.beg);
        ...
        zeKernelSuggestGroupSize(kernels[2], items, items, 1U, &groupSizeX, &groupSizeY, &groupSizeZ);
        zeKernelSetGroupSize(kernels[2], groupSizeX, groupSizeY, groupSizeY);

        // Push arguments
        zeKernelSetArgumentValue(kernels[2], 0, sizeof(&sharedA), &sharedA);
        zeKernelSetArgumentValue(kernels[2], 1, sizeof(sharedB), &sharedB);
        zeKernelSetArgumentValue(kernels[2], 2, sizeof(dstResultSum), &dstResultSum);
        ...
        // Immediately submit a kernel to the device and launch
        zeCommandListAppendLaunchKernel(cmdList, kernels[2], &launchArgs, event, 0, nullptr);
    ...
    if (file.is_open()) {
        ...
        ze_kernel_desc_t kernelDesc = {};
        kernelDesc.pKernelName = "incrementandsum";
        zeKernelCreate(module, &kernelDesc, &kernels[3]);

        zeKernelSuggestGroupSize(kernels[3], items, items, 1U, &groupSizeX, &groupSizeY, &groupSizeZ);
        zeKernelSetGroupSize(kernels[3], groupSizeX, groupSizeY, groupSizeY);

        // Push arguments
        zeKernelSetArgumentValue(kernels[3], 0, sizeof(&sharedA), &sharedA);
        zeKernelSetArgumentValue(kernels[3], 1, sizeof(sharedB), &sharedB);
        zeKernelSetArgumentValue(kernels[3], 2, sizeof(dstResultSum), &dstResultSum);
        ...
        // Immediately submit a kernel to the device and launch
        zeCommandListAppendLaunchKernelIndirect(cmdList, kernels[3], &launchArgs, event, 0, nullptr);
    ...
    if (file.is_open()) {
        ...
        // Immediately submit a kernel to the device and launch
        zeCommandListAppendLaunchKernel(cmdList, kernels[3], &launchArgs, event, 0, nullptr);
